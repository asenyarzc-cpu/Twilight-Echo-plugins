...
try {
  ...
  try {
    QRCode = require(require.resolve('qrcode', { paths: [process.cwd(), __dirname] }))
  } catch (error) {
    throw new Error('汽水扫码需要 qrcode 组件，但 Twilight Echo 当前运行环境没有提供 qrcode：' + (error?.message || String(error)))
  }
}
